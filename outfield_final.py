...
def display_player_stats(player_name, stats_json): # Show formatted player stats based on their position
    ...
    # Print player header
    print("\n" + "="*80)
    print(f" Stats for {player_name} ({team_name}):")
    print("="*80)
...
def display_team_stats(team_name, stats_json):
    ...
    # Pulls relevant team stats
    games_played  = stat.get('gamesPlayed', 'N/A') 
    hits          = stat.get('hits', 'N/A')
    rbi           = stat.get('rbi', 'N/A')
    runs          = stat.get('runs', 'N/A')
    ...
    print(f"Runs Batted In  : {rbi}")
    print(f"Home Runs       : {home_runs}")
    print(f"Batting Avg.    : {avg}")
    print(f"On-Base Pct.    : {obp}")
    print(f"Stolen Bases    : {stolen_bases} ({float(sb_pct)*100:.1f}%)")
