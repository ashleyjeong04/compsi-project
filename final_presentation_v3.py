...
def fetch_and_cache_valid_entries():
    ...
    for team in teams_list: # Loops through teams 
        ...
        # 3. Extracts and stores each player's ID and full name
    ...
    if is_file_up_to_date(VALID_ENTRIES_FILE, TRADE_DEADLINE): # Checks if the file is up to date
        try:
            with open(VALID_ENTRIES_FILE, 'r') as f:
                return json.load(f) # Returns the loaded file if valid
...
def categorize_score(score): # Converts numeric sentiment score into a understandable labels 
    if score <= -0.667: # Defines ranges for the range of sentiments, from strongly negative to strongly positive
...
def store_articles(articles, db_path="news_articles.db"): # Stores articles to a local SQLite database file
    try:
        ...
        for article in articles: # Loops through each article 
            ...
            )) # After inserting each article into the SQLite database, extracts relevant fields
        conn.commit()
...
def fetch_player_stats(player_id): # Fetch player stats from the MLB API 
    ...
    try:
        ...
        print(f"Error fetching player stats: {e}") # Handle network or HTTP errors 
        return {}
...
def fetch_team_stats(team_id): # Get a team's seasonal performance stats 
...
def display_results(entity_name, entity_type, stats, articles): # Display stats based on entity type 
    ...
    if articles:
        ...
        # Determine start and end of article date range
        if dates:
        ...
        for idx, article in enumerate(articles, start=1):
            ...
            url   = article.get('url', '') # Article URL
        ...
        # Plot hhistogram of sentiment Distribution
...
# 6. Putting it together, main flow of program
